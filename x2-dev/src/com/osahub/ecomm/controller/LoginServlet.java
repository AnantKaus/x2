...
public class LoginServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		...
		if(product == null){
			ProductDao i = new ProductDao(productCode, image1, category,  name, description, "admin@swatidesignerwear.com", new Date().toString());
			//Now save the object  
			ofy().save().entities(i);
			ofy().clear();
	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp) throws IOException {
		session = req.getSession();
		//cheeck if user already registered or not
		session.setAttribute("loggedInUserEmail", "anshul@osahub.com");
		session.setAttribute("loggedInUserName", "Anshul Jain");
